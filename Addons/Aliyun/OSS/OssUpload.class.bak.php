...
class OssUpload {
	...
	protected function _set_oss_client() {
		$this->_client = new \ALIOSS ( $this->_AccessKeyId, $this->_AccessKeySecret, $this->_Endpoint );
		$this->_client->get_enable_domain_style ( $this->_domian );
		return $this->_client;
	}
	...
	public function save(&$file, $replace = false) {
		...
		if ($file ['size'] <= (20 * 1024 * 1024)) { // 小于2M用upload_file_by_content方法
			...
			$options ['content'] = file_get_contents ( $file ['tmp_name'] );
			$result = $this->_client->upload_file_by_content ( $this->_Bucket, $file ['name'], $options );
			var_dump($result);
		} else if (($file ['size'] > (2 * 1024 * 1024)) && ($file ['size'] < (100 * 1024 * 1024))) { // 小于2M~100M用upload_file_by_content方法
			$options = array ();
			$result = $this->_client->upload_file_by_file ( $this->_Bucket, $file ['name'], $file ['tmp_name'], $options );
	...
	public function list_bucket(){
		return $this->_client->list_bucket();
	}
