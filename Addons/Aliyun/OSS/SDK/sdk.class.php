...
class ALIOSS {
	 ...
	 * @param string $bucket
	 *        	(Required)
	 * @param string $index_document
	 *        	(Required)
	 * @param string $error_document
	 *        	(Optional)
	 * @param array $options
	 *        	(Optional)
	 * @throws OSS_Exception
	 ...
	 * @param string $bucket
	 *        	(Required)
	 * @param bool $is_allow_empty_referer
	 *        	(Required)
	 * @param array $referer_list
	 *        	(Optional)
	 * @param array $options
	 *        	(Optional)
	 * @return ResponseCore
	...
	public function copy_object($from_bucket, $from_object, $to_bucket, $to_object, $options = NULL) {
		...
		if (isset ( $options [self::OSS_HEADERS] )) {
			...
			$options [self::OSS_HEADERS] = array (
					self::OSS_OBJECT_COPY_SOURCE => '/' . $from_bucket . '/' . $from_object 
			);
		}
	 ...
	 *        	(Required) uploadId
	 * @param
	 *        	array | xml $parts 可以是一个上传成功part的数组，或者是一个ReponseCore对象
	 * @param array $options
	 *        	(Optional) Key-Value数组
	 * @return ResponseCore
	 */
	 ...
	 *        	是否递归，如果为true，则递归读取所有目录，默认为不递归读取
	 ...
	 * @param string $bucket
	 *        	(Required) Bucket名称
	 * @param array $object_arry
	 *        	(Required) object数组，所有的object必须在同一个bucket下
	...
	public function get_object_group($bucket, $object_group, $options = NULL) {
		...
		$options [self::OSS_OBJECT] = $object_group;
		$options [self::OSS_HEADERS] = array (
				self::OSS_OBJECT_GROUP => self::OSS_OBJECT_GROUP 
	...
	public function auth($options) {
		...
		if (isset ( $options [self::OSS_OBJECT] ) && '/' !== $options [self::OSS_OBJECT]) {
			$signable_resource = '/' . str_replace ( array (
					'%2F',
					'%25' 
			), array (
					'/',
