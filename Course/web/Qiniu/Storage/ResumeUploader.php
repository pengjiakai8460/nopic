...
final class ResumeUploader
{
    ...
    public function explodeUpToken($upToken)
    {
        ...
        $accessKey = $items[0];
        $putPolicy = $this->base64_urlSafeDecode($items[2]);

        $scope = json_decode($putPolicy,true)['scope'];
        //var_dump($scope);exit();
    ...
    private function fileUrl($fname)
    {
        ...
        $url .= '/mimeType/' . self::base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . self::base64_urlSafeEncode($this->key);
        }
        $url .= '/fname/' . self::base64_urlSafeEncode($fname);
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val = self::base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
